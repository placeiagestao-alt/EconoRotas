...
let _dbConnectPromise: Promise<any> | null = null;
let _lastDbConnectAttempt = 0;
let _lastDbConnectionError: string | null = null;
let _schemaHealthCache: {
  checkedAt: number;
  value: Awaited<ReturnType<typeof readDatabaseSchemaHealth>>;
} | null = null;

const DB_CONNECT_RETRY_MS = 30_000;
const DB_SCHEMA_HEALTH_CACHE_MS = 60_000;
...
function hydrateMemory(data: any) {
  ...
  memory.routeSchedules = Array.isArray(data.routeSchedules)
    ? data.routeSchedules
    : [];
  memory.routeHistory = Array.isArray(data.routeHistory)
    ? data.routeHistory
    : [];
  memory.chatHistory = Array.isArray(data.chatHistory) ? data.chatHistory : [];
  memory.userIntegrations = Array.isArray(data.userIntegrations)
    ? data.userIntegrations
    : [];
  memory.operationalEvents = Array.isArray(data.operationalEvents)
    ? data.operationalEvents
    : [];
  memory.routeMetrics = Array.isArray(data.routeMetrics)
    ? data.routeMetrics
    : [];
  memory.optimizationJobs = Array.isArray(data.optimizationJobs)
    ? data.optimizationJobs
    : [];
  memory.performanceBenchmarks = Array.isArray(data.performanceBenchmarks)
    ? data.performanceBenchmarks
    : [];
  memory.geocodeCache = Array.isArray(data.geocodeCache)
    ? data.geocodeCache
    : [];
  memory.addressCorrections = Array.isArray(data.addressCorrections)
    ? data.addressCorrections
    : [];
...
export async function getLocationCommercialCache(data: {
  ...
  const cutoff = new Date(
    Date.now() - (data.ttlDays ?? 30) * 24 * 60 * 60 * 1000
  );
...
async function shouldUseMemoryDb() {
  ...
  if (
    ENV.hasInvalidProductionDatabaseUrl &&
    !hasPersistentFallbackDbConfigured()
  ) {
    return false;
  }
...
function formatDatabaseUnavailableMessage() {
  const details = _lastDbConnectionError
    ? ` Detalhe: ${_lastDbConnectionError}`
    : "";
...
async function getDatabaseSchemaHealth() {
  ...
  if (
    _schemaHealthCache &&
    now - _schemaHealthCache.checkedAt < DB_SCHEMA_HEALTH_CACHE_MS
  ) {
...
function sortByDateAsc<T extends Record<string, any>>(
  items: T[],
  field: string
) {
  return [...items].sort(
...
export async function createRoute(
  ...
  const inserted = await db.insert(routes).values({
    ...
    startLocation: data.startLocation ?? null,
    startLatitude:
      data.startLatitude !== undefined ? String(data.startLatitude) : null,
    startLongitude:
      data.startLongitude !== undefined ? String(data.startLongitude) : null,
    endLocation: data.endLocation ?? null,
    endLatitude:
      data.endLatitude !== undefined ? String(data.endLatitude) : null,
    endLongitude:
...
export async function createStops(
    ...
    if (await shouldUseMemoryDb()) {
      ...
      const createdStops = stopsData.map(stop => {
        ...
        return {
          ...
          longitude:
            stop.longitude !== undefined ? String(stop.longitude) : null,
          ...
          commercialDetectionStatus:
            stop.commercialDetectionStatus ?? "unknown",
          ...
          commercialLastCheckedAt: stop.commercialLastCheckedAt
            ? new Date(stop.commercialLastCheckedAt)
            : null,
          createdAt: now,
        };
      });
  ...
  const values = stopsData.map(s => {
    ...
    return {
      ...
      commercialLastCheckedAt: s.commercialLastCheckedAt
        ? new Date(s.commercialLastCheckedAt)
        : null,
    };
  });
    ...
    if (await shouldUseMemoryDb()) {
      ...
      const normalizedMetadata =
        data.metadata !== undefined
          ? normalizeStopMetadata(data.metadata)
          : undefined;
...
function routeExecutionStatusForEvent(
  type: string
): RouteExecutionStatus | null {
  const normalized = normalizeExecutionEventType(type);
  if (
    normalized === "route_started" ||
    normalized === "route_paused" ||
    normalized === "route_resumed"
  ) {
    return "started";
  }
  ...
  if (!db) {
    if (await shouldUseMemoryDb()) {
      ...
      if (status === "started") {
        ...
        const startedAt = metric.startedAt
          ? new Date(metric.startedAt).getTime()
          : NaN;
    ...
    if (await shouldUseMemoryDb()) {
      return sortByDateDesc(memory.operationalEvents, "createdAt")
        ...
        .map(event => ({
          ...event,
          userName:
            memory.users.find(user => user.id === event.userId)?.name ?? null,
          userEmail:
            memory.users.find(user => user.id === event.userId)?.email ?? null,
          routeName:
            memory.routes.find(route => route.id === event.routeId)?.name ??
            null,
        }));
    }
    requireConfiguredDatabase();
...
export async function getLatestRouteOptimizationEvent(
  routeId: number,
  userId: number
) {
...
function extractCityFromAddress(address: string) {
  ...
  if (parts.length >= 2) {
    const stateLike = parts.findIndex(part =>
      /\bSP\b|\bSao Paulo\b/i.test(part)
    );
...
export async function getOptimizationJobById(id: number) {
  ...
  if (!db) {
    if (await shouldUseMemoryDb()) {
      return (
        memory.optimizationJobs.find(item => Number(item.id) === id) ?? null
      );
    }
    requireConfiguredDatabase();
  }
  ...
  if (!db) {
    if (await shouldUseMemoryDb()) {
      ...
      const failedJobs = memory.optimizationJobs.filter(
        job =>
          job.status === "failed" && new Date(job.createdAt).getTime() >= cutoff
      );
...
function buildLongRunningJobAlerts(
  runningJobs: any[],
...
async function hasRecentQueueIntegrityEvent(
  type: string,
  title: string,
  minutes = 30
) {
...
export async function createPerformanceBenchmark(
  data: CreatePerformanceBenchmarkInput
) {
  ...
  const osrmFailureRate = roundMetric(
    metricPercent(
      routeMetricsRows.reduce(
        (total, row) =>
          total + Number(row.osrmFailureCount ?? row.osrm_failure_count ?? 0),
        0
      ),
      routeMetricsRows.reduce(
        (total, row) =>
          total + Number(row.osrmCallCount ?? row.osrm_call_count ?? 0),
        0
      )
    )
  );
  ...
  return {
    ...
    pipeline: {
      ...
      correctionMsAverage: Math.round(
        metricAverage(
          routeMetricsRows.map(row =>
            Number(row.correctionMs ?? row.correction_ms ?? 0)
          )
        )
      ),
      optimizerMsAverage: Math.round(
        metricAverage(
          routeMetricsRows.map(row =>
            Number(row.optimizerMs ?? row.optimizer_ms ?? 0)
          )
        )
      ),
...
export async function getGoLive500Dashboard(days = 30) {
  ...
  const performanceBenchmarks =
    await getPerformanceBenchmarkDashboard(safeDays);
...
function buildRouteMetricsSummary(metrics: any[], days: number) {
  ...
  const osrmFallback = metrics.filter(metric =>
    Boolean(metric.osrmFallback)
  ).length;
  const revisits = metrics.reduce(
    (totalCount, metric) =>
      totalCount + Number(metric.regionRevisitedCount || 0),
    0
  );
  ...
  const geocodingScoreDistribution = {
    ...
    good: geocodingAverageScores.filter(score => score >= 75 && score < 90)
      .length,
    attention: geocodingAverageScores.filter(score => score >= 60 && score < 75)
      .length,
  ...
  const detectedIssues = metrics.reduce(
    (totalCount, metric) =>
      totalCount + Number(metric.issuesDetectedCount || 0),
    0
  );
  const correctedIssues = metrics.reduce(
    (totalCount, metric) =>
      totalCount + Number(metric.issuesCorrectedCount || 0),
    0
  );
  ...
  const issuesRemaining = metrics.reduce(
    (totalCount, metric) =>
      totalCount + Number(metric.issuesRemainingCount || 0),
    0
  );
  const batchCorrections = metrics.reduce(
    (totalCount, metric) =>
      totalCount + Number(metric.batchCorrectionCount || 0),
    0
  );
  ...
  const executionStarted = metrics.filter(metric =>
    ["started", "completed", "abandoned"].includes(
...
function buildExecutionReportPeriod(
  ...
  const durations = metrics
    .map(metric => Number(metric.executionDurationMs || 0))
    .filter(value => Number.isFinite(value) && value > 0);
  const blockedByReason = blockedEvents.reduce<Record<string, number>>(
    (acc, event) => {
...
const EXECUTIVE_OBSERVABILITY_EVENT_TYPES = [
...
function countUniqueEventRoutes(events: any[], type: string) {
  const routeIds = new Set<number | string>();
...
function getMetricOperationalStatus(metric: any) {
  const metadata = parseMetricMetadata(metric.metadata);
  return String(
...
function isFarGpsStopAction(event: any) {
  const metadata = parseOperationalMetadata(event.metadata);
  const integrity = String(metadata.locationIntegrity || "");
  return (
    integrity === "far_from_stop" ||
    integrity === "remote_confirmation" ||
    metadata.remoteConfirmation === true
  );
}

function buildExecutiveObservabilityPeriod(
  metrics: any[],
  events: any[],
  days: number
) {
  ...
  const stopActionEvents = events.filter(event =>
    ["route_stop_delivered", "route_stop_failed"].includes(String(event.type))
  );
  ...
  const farGpsFromStopActions =
    stopActionEvents.filter(isFarGpsStopAction).length;
  ...
  const remoteConfirmationsFromStopActions = stopActionEvents.filter(event => {
    const metadata = parseOperationalMetadata(event.metadata);
    return (
      event.type === "route_stop_delivered" &&
      metadata.remoteConfirmation === true
    );
  }).length;
  ...
  return {
    ...
    averageOptimizationRuntimeMs:
      routeMetricsSummary.averageOptimizationRuntimeMs,
    averageOptimizationRuntimeSeconds:
      routeMetricsSummary.averageOptimizationRuntimeSeconds,
  };
}
...
function buildExecutiveObservabilityComparison(
  last7Days: any,
  last30Days: any
) {
  const weeklyPace = (value30: number) => (Number(value30 || 0) / 30) * 7;
  const signals = [
...
async function getExecutiveObservabilityEvents(days: number) {
  ...
  if (!db) {
    if (await shouldUseMemoryDb()) {
      ...
      return sortByDateDesc(
        memory.operationalEvents.filter(
          event =>
            EXECUTIVE_OBSERVABILITY_EVENT_TYPES.includes(event.type as any) &&
            new Date(event.createdAt).getTime() >= cutoff
        ),
        "createdAt"
      );
...
export async function getExecutiveObservabilityReport() {
...
function buildManualCorrectionsSummary(corrections: any[]) {
  ...
  for (const correction of corrections) {
    ...
    const city =
      correction.city ||
      extractCityFromAddress(String(correction.correctedAddress || ""));
    if (address)
...
function buildGeocodingWindowSummary(args: {
  ...
  return {
    ...
    suspiciousStops:
      routeMetricsSummary.geocodingConfidence.suspiciousStopCount,
...
function buildRouteQualityDashboard(events: any[]) {
  const routeEvents = events.filter(event =>
    String(event.type || "").startsWith("route_")
  );
  const scores: number[] = [];
  let corrections = 0;
  ...
  for (const event of events) {
    ...
    if (event.type === "geocoding_cache_hit") {
      ...
      if (
        ...
        !metadata.geocoding_cache_hit_backend
      ) {
        backendHits += 1;
      }
    }
    if (
      event.type === "geocoding_cache_miss" &&
      !metadata.geocoding_cache_miss
    ) {
      misses += 1;
    }
  }
...
async function buildAdminOperationalDashboardLive() {
  ...
  if (!db) {
    if (await shouldUseMemoryDb()) {
      ...
      const recentRoutes = sortByDateDesc(memory.routes, "createdAt").slice(
        0,
        8
      );
      ...
      const executiveObservability = await getExecutiveObservabilityReport();
...
export async function getUserStats(userId: number, days = 30) {
  ...
  if (!db) {
    if (await shouldUseMemoryDb()) {
      ...
      const distances = userRoutes.map(route =>
  ...
  const totalDistance = await db
    .select({ sum: sql`SUM(totalDistance)` })
    .from(routes)
    .where(and(eq(routes.userId, userId), gte(routes.createdAt, cutoffDate)));

  const avgTime = await db
    .select({ avg: sql`AVG(totalTime)` })
    .from(routes)
    .where(and(eq(routes.userId, userId), gte(routes.createdAt, cutoffDate)));
  ...
  if (!db) {
    if (await shouldUseMemoryDb()) {
      ...
      for (const history of memory.routeHistory) {
        if (
          history.userId !== userId ||
          new Date(history.executedDate) < startDate
        ) {
          continue;
        }
