...
Maps:
- `client/src/components/MapView.jsx` - OpenStreetMap map powered by Leaflet/react-leaflet. Supports dynamic markers, popups, route polylines, responsive zoom, and dark-mode styling hooks.
- `client/src/services/maps/locationService.ts` - Location utilities for centers, geolocation, distance, and future realtime tracking.
- `client/src/services/maps/markerService.ts` - Marker normalization and Leaflet icon configuration for React/Vite.
...
Maps use OpenStreetMap tiles through Leaflet and do not require a provider API key.

**Default:** import `MapView` from `client/src/components/MapView.jsx` or `client/src/components/Map.tsx`.

**Implementation:**
- Frontend: `MapView.jsx` renders `MapContainer`, `TileLayer`, markers, popups, and route polylines.
- Location utilities: `client/src/services/maps/locationService.ts`.
- Marker utilities: `client/src/services/maps/markerService.ts`.

The map layer is prepared for future routing engines such as OSRM/Valhalla, clustering, heatmaps, realtime vehicle tracking, geolocation, and multiple users online.
